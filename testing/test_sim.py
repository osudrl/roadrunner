...
def test_sim_viewer(sim):
    ...
    test_sim = sim()
    test_sim.reset()
    h = test_sim.hfield_generator.block()
    test_sim.randomize_hfield(data=h)
    test_sim.set_base_position(np.array([0, 0, 1.0]))
    test_sim.hold()
    ...
    while render_state:
        ...
        if not test_sim.viewer_paused():
            for _ in range(50):
                test_sim.sim_forward()
                d = test_sim.data.sensor('left-toe').data[:]
                print(d)
                # d = 0.0 < test_sim.data.sensor('left-range-0').data < 0.02
                # print(test_sim.data.sensor('left-range-0').data, "touched or not" , d)
        render_state = test_sim.viewer_render()
