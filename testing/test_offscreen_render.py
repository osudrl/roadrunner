...
def test_offscreen_rendering():
    ...
    OFFSCREEN = False#input("Offscreen rendering? (y/n): ").lower() == 'y'
    TERRAIN = 'flat'#input("Terrain? (flat/hfield): ").lower()
    ...
    size = [25, 50, 100, 150, 200, 250, 300, 350, 400, 480] if OFFSCREEN else [[848, 480]]
    sim_time_avg_list = []
    ...
    for s in size:
        ...
        sim.reset()
        if TERRAIN == 'flat':
            # 18.25, 22, 12.5 in box
            sim.geom_generator._create_geom('box0', *[0.80, 0, 0], rise=0.31115, length=0.4572/2, width=0.5588/2)
        elif TERRAIN == 'hfield':
        ...
        sim.set_base_position(np.array([0, 0, 1.2827]))
        ...
        while render_state:
            ...
            if not OFFSCREEN:
                ...
                clip_range = [0, 2]
                depth = np.clip(depth, *clip_range)
