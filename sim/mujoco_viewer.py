...
class MujocoViewer():
    def __init__(self,
        ...
        # assert len(self.reset_qpos) == self.model.nq - 4, \
        #        f"{FAIL}Size of MujocoViewer reset qpos ({len(self.reset_qpos)}) does not match model nq size ({self.model.nq}).{ENDC}"
        ...
        self._image_path = os.path.join(os.getcwd(), "screenshots", "frame_%07d.png")
        ...
        self.hfield_marker_ids = {}

        # Buffer used for recording video
        self._record_frame = None
        self._record_video = record_video
        if self._record_video:
            self._rgb_buffer = np.empty((self.viewport.height, self.viewport.width, 3),
                                        dtype=np.uint8)
            self._video_writer = imageio.get_writer(
                'sim-video-'+time.strftime("%Y%m%d-%H%M%S")+'.mp4', fps=50)
        ...
        with self._gui_lock:
            ...
            if self._showinfo:
                ...
                mj.mjr_overlay(mj.mjtFontScale.mjFONTSCALE_150.value,
                               ...
                               self.ctx)
            if self._record_video:
                mj.mjr_readPixels(self._rgb_buffer, None, self.viewport, self.ctx)
                self._record_frame = np.flipud(self._rgb_buffer)
                self._video_writer.append_data(self._record_frame)
    ...
    def _key_callback(self, window, key, scancode, action, mods):
        if action == glfw.RELEASE:
            ...
            if key == glfw.KEY_P and mods == 0:
                ...
                os.makedirs(os.path.dirname(self._image_path % self._image_idx), exist_ok=True)
    ...
    def render_hfield(self,
                      hfield_map,
                      local_grid_rotated,
                      rgba: np.ndarray = np.array([0.8, 1, 0.8, 1.0]),
                      name: str = "pelvis"):
        # Define the rotation, size, and color of the point markers
        so3 = euler2so3(z=0, x=0, y=0)
        size = [0.015, 0.015, 0.005]

        if name not in self.hfield_marker_ids:
            self.hfield_marker_ids[name] = []
        ids = self.hfield_marker_ids[name]

        for i in range(len(hfield_map)):
            if i < len(ids):
                x, y, z = local_grid_rotated[i][0], local_grid_rotated[i][1], hfield_map[i] + 0.02
                self.update_marker_position(ids[i], [x,y,z])
            else:
                x, y, z = local_grid_rotated[i][0], local_grid_rotated[i][1], hfield_map[i] + 0.02
                id = self.add_marker("sphere", "", [x,y,z], size, rgba, so3)
                ids.append(id)
