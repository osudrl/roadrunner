...
def compute_done(self):
    ...
    height_limit = 0.65 if "cassie" in self.__class__.__name__.lower() else 0.85
    if np.abs(base_euler[1]) > 20/180*np.pi or np.abs(base_euler[0]) > 20/180*np.pi or base_height < 0.5:
        return True
    else:
        return False
