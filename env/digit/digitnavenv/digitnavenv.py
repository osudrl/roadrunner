...
class DigitNavEnv(DigitEnv):
    ...
    def get_mirror_dict(self, device):
        privilege_state_mirror_indices = torch.tensor(self.get_state_mirror_indices(), dtype=torch.long,
                                                      device=device)
        state_mirror_indices = privilege_state_mirror_indices.clone()
        action_mirror_indices = torch.tensor(self.get_action_mirror_indices(), dtype=torch.long, device=device)
        mirror_dict = {
            'state_mirror_indices': {
                'base': state_mirror_indices,
                'privilege': privilege_state_mirror_indices
            },
    ...
    def get_state(self):
        ...
        if self.clock_type:
            out.extend([[self.clock.get_swing_ratios()[0], 1 - self.clock.get_swing_ratios()[0]],
        ...
        return dict(base=out, privilege=out)
