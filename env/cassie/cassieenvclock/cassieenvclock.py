...
class CassieEnvClock(CassieEnv):
    ...
    def reset_info(self, difficulty=None):
        ...
        # if self.mix_terrain:
        #     self.hfield_name = np.random.choice(['stair-small', 'block', 'bump'], p=[0.4, 0.4, 0.2])
        self.hfield_name = 'platform'
        self._init_orient_add = [-np.pi, np.pi] # rad
        match self.hfield_name:
            case 'stair':
            ...
            case 'stair-small':
                base_position = self.sim.get_base_position()
                base_position[0] = np.random.uniform(-5, -3)
                self.sim.set_base_position(position=base_position)
                height_map = self.sim.hfield_generator.create_stair_even(difficulty=0.5)
                self.x_velocity = np.random.uniform(0.2, 1)
            case 'bump':
                base_position = self.sim.get_base_position()
                base_position[0] = -5
                self.sim.set_base_position(position=base_position)
                height_map = self.sim.hfield_generator.create_bump(difficulty=1)
            case 'stone':
                base_position = self.sim.get_base_position()
                base_position[0] = -6
                self.sim.set_base_position(position=base_position)
            ...
            case 'noisy':
                ...
                base_position[0] = -3
                self.sim.set_base_position(position=base_position)
                terrain = np.random.choice(["flat", "noisy"], p=[0.1, 0.9])
                height_map = getattr(self.sim.hfield_generator, f"create_{terrain}")()
            case 'block':
                base_position = self.sim.get_base_position()
                base_position[0] = 0
                self.sim.set_base_position(position=base_position)
        ...
        # Always have flat cases
        if np.random.uniform() < 0.05:
            height_map = self.sim.hfield_generator.create_flat()
            # Commands to go backwards if flat terrain
            self.x_velocity = np.random.uniform(*[-0.5, 0.5])
    ...
    def get_proprioceptive_state(self):
        ...
        self.robot_estimator_state.pelvis.translationalVelocity[:],
        self.robot_estimator_state.pelvis.rotationalVelocity[:]
        ))
        return out.tolist()

    def get_proprioceptive_state(self, include_joint: bool = False):
        # base_linear_acceleration = self.sim.get_body_acceleration(self.sim.base_body_name)[:3]
        # base_linear_acceleration[2] -= 9.81 # remove gravity
        # base_angular_velocity = self.sim.get_base_angular_velocity()
        base_linear_acceleration = self.robot_estimator_state.pelvis.translationalVelocity[:]
        base_angular_velocity = self.robot_estimator_state.pelvis.rotationalVelocity[:]
        out = np.concatenate((base_linear_acceleration, base_angular_velocity))
        # Check to see if we want to include joint angles and velocities
        if include_joint:
