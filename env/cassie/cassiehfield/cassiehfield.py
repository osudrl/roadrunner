...
class CassieHfield(CassieEnv):
    ...
    def __init__(self,
        ...
        if self.autoclock or self.autoclock_simple:
            ...
            self._swing_ratio_bounds = [0.5, 0.5]
            self._cycle_time_bounds = [0.7, 0.7]
        self.command_randomize_freq_bounds = [200, 250] # in terms of trajectory steps
        ...
        self.observation_size = 35#len(self.get_robot_state())
    ...
    def reset_info(self, difficulty=None, env_probs=None):
        base_position = self.sim.get_base_position()
        # Randomize terrain, robot position, and update height map/depth image
        if self.mix_terrain:
            hfield_name = np.random.choice(['flat', 'noisy', 'stair', 'bump', 'platform'],
                                           p=env_probs)
        else:
            hfield_name = self.hfield_name

        self.hfield_name_real = hfield_name
        # hfield_name = 'stair'
        match hfield_name:
            case 'stair':
                ...
                height_map = self.sim.hfield_generator.create_stair_even(difficulty=difficulty)
            ...
            case 'bump':
                ...
                self.sim.set_base_position(position=base_position)
                difficulty = 1 if difficulty is None else difficulty
                height_map = self.sim.hfield_generator.create_bump(difficulty=difficulty)
                self._x_velocity_bounds = [0.3, 1.0]
                self._y_velocity_bounds = [0.0, 0.0]
            case 'platform':
                base_position[0] = 0
                ...
                difficulty = 1 if difficulty is None else difficulty
                height_map = self.sim.hfield_generator.create_platform(difficulty=difficulty,
            ...
            case 'flat':
                ...
                self.y_velocity = np.random.uniform(*[-0.3, 0.3])
            case _:
                raise NotImplementedError(f"Terrain {hfield_name} not implemented")
        ...
        for (x,y,z) in [self.sim.get_site_pose(self.sim.feet_site_name[0])[:3],
            ...
            tl_hgt = self.sim.get_hfield_height(x + 0.09, y + 0.05)
            tr_hgt = self.sim.get_hfield_height(x + 0.09, y - 0.05)
            bl_hgt = self.sim.get_hfield_height(x - 0.09, y + 0.05)
            br_hgt = self.sim.get_hfield_height(x - 0.09, y - 0.05)
    ...
    def reset(self, difficulty=None, interactive_evaluation=False, env_probs=None):
        ...
        self.reset_info(difficulty=difficulty, env_probs=env_probs)
    ...
    def step(self, action: np.ndarray):
        ...
        for i, b in enumerate(self.sim.body_collision_list[:3]):
        ...
        done = done and self.traj_idx > 30
        ...
        return self.get_state(), r, done, dict(foot_scuffed=foot_scuffed,
                                               out_of_map=out_of_map,
                                               body_collision_with_env=any(self.body_collision_with_env),
                                               torque=np.abs(torque).sum())
    ...
    def update_clock(self, clock_action):
        if self.autoclock_simple:
            ...
            # shift_left  = 0.0 + shift_add_left
            # shift_right = 0.5 + shift_add_right
        else:
            phase_add, shift_add_left, shift_add_right = clock_action[0], clock_action[1], clock_action[2]
            # Scale phase add by multiplier
            phase_add = 1 / self.default_policy_rate * self.scale_nn(phase_add, 0.5, 1.0, smoothness=3)
            shift_left  = 0.0 + self.scale_nn(shift_add_left, -0.5, 0.5, smoothness=3)
            shift_right = 0.5 + self.scale_nn(shift_add_right, -0.5, 0.5, smoothness=3)
    ...
    def randomize_commands(self, init=False):
        ...
        mode = np.random.choice(choices, p=[0.0, 0.0, 0.0, 0.6, 0.4])
        match mode:
            case 'in-place':
    ...
    def update_markers(self):
        ...
        if self.sim.viewer is not None:
            ...
            if hasattr(self, 'hfield_ground_truth'):
                self.sim.viewer.render_hfield(self.hfield_ground_truth, self.local_grid_rotated)
            # self.sim.viewer.render_hfield(self.hfield_map_realtime, self.local_grid_rotated, rgba=np.array([0, 0, 0.5, 1.0]), name='realtime')
